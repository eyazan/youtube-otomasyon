...
// simdi'den en az minOnceSaat sonraki ilk hourUTC:00 anı
function sonrakiSlot(simdi, hourUTC, minOnceSaat = 1) {
  const t = new Date(Date.UTC(simdi.getUTCFullYear(), simdi.getUTCMonth(), simdi.getUTCDate(), hourUTC, 0, 0));
  while (t.getTime() - simdi.getTime() < minOnceSaat * 3600000) t.setUTCDate(t.getUTCDate() + 1);
  return t;
}
